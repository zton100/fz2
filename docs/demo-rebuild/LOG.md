...
## 2026-07-07 Demo 后续硬化：50 层长线和特殊词缀验证

### 本轮目标

- 将长线平衡 smoke 从 30 层继续扩展到 50 层。
- 在实际战斗掉落路径里统计词缀分类，验证 16 层后特殊词缀可以进入掉落结果。

### 本轮完成

- `smokebalance` 新增词缀分类计数：基础、衍生、特殊。
- 长线模拟拆成两段：
...
- 下一步可以把 `smokebalance` 的断言拆成可单测的纯函数，减少命令行 smoke 继续膨胀。
- 50 层后需要开始观察 Artifact 稀有度和更高层 Boss 尖峰。
...
- 继续推进 50 层长线 smoke 后的维护性硬化。
- 将 `smokebalance` 中的失败判定从 `main()` 拆成可单测的纯规则函数，降低后续调参时的维护成本。

### 本轮完成

- 新增 `server/cmd/smokebalance/checks.go`：
  - 早期 10 层循环规则。
  - 转生魂点规则。
  - 1 -> 30 长线规则。
  - 30 -> 50 深层规则。
...
- 下一步可以继续把 `runCycle` 的自动穿戴、分解、强化策略拆出测试，或开始观察 50 层后 Artifact 稀有度曲线。

### 提交信息

- 待提交。

## 2026-07-07 Demo 后续硬化：平衡 smoke 策略拆分

### 本轮目标

- 继续推进 `smokebalance` 维护性硬化。
- 将 `runCycle` 里的自动穿戴、分解、强化策略从主流程拆出，并补可定位的策略单测。

### 本轮完成

- 新增 `server/cmd/smokebalance/strategy.go`：
  - 自动穿戴只接受正战力收益装备。
  - 自动分解弱装并把材料计入玩家库存。
  - 自动强化按可负担装备的战力收益 / 材料成本选择目标。
  - 战力收益计算集中在策略层。
- 新增 `strategy_test.go`：
  - 覆盖只穿戴提升装备。
  - 覆盖弱装分解和强装保留。
  - 覆盖强化目标选择最高性价比装备。
  - 覆盖自动强化消耗材料并提升战力。
- `main.go` 进一步收敛为模拟编排、指标采集和输出。
- 自动穿戴策略跳过 nil 装备，避免异常背包数据导致 smoke panic。
...
- 下一步可以继续观察 50 层后 Artifact 稀有度曲线，或把平衡阈值配置化，减少调参时改代码的频率。
...
- 下一步需要把 30 层以后的目标拆成更细指标：31 -> 50 层、首次特殊词缀出现、以及 Boss 首通奖励重复边界。
...
## 2026-07-09 Demo 后续硬化：装备界面游戏化

### 本轮目标

- 继续解决“界面不像游戏”的问题，优先升级核心装备决策区。
- 将背包、当前穿戴、装备详情从信息表格感改成暗黑 RPG 装备卡片。
- 保持现有装备、锁定、穿戴、分解、强化继承等交互完整可用。

### 本轮完成

- 背包页：
  - 背包列表升级为装备卡片，显示装备图标、品质标签、部位、强化等级、评分和提升状态。
  - 锁定、穿戴、分解按钮收敛到卡片右侧操作列。
  - 当前选中装备用更明显的金色底边强调。
- 当前穿戴：
  - 穿戴装备格从三列小信息块改为两列装备槽卡。
  - 每个槽位展示图标、部位、装备名、评分、品质和强化等级。
  - 空槽保留等待掉落提示。
- 装备详情：
  - 当前选择 / 已装备改成带图标、品质徽章、部位徽章、强化徽章的结构化面板。
  - 词缀从大段文本改成逐条属性行。
  - 对比结果改成带正负颜色边线的差异行。
